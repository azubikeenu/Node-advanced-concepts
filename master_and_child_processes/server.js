...
if (cluster.isMaster) {
  ...
  const worker1 = require('child_process').fork('./workers/worker_1');
  const worker2 = require('child_process').fork('./workers/worker_2');
  ...
  // listens for messages from the worker2 node
  ...
  /**
   * When worker is created it emits an online event to the master node
   * So we are using the cluster to send events to our child processes
   */
  cluster.on('online', (worker) => {
    console.log(`Message received from - ${worker.process.pid}`);
    // the worker recieves this event by listening to the message event
    worker.on('message', (num) => {
      if (num % 2 === 0) {
        // sends messages to the worker1 node and delegates the handling to the worker node
        worker1.send(num);
      } else {
        worker2.send(num);
      }
